"""Tests for the lookout validate CLI command."""
...
from lookout.discovery.models import DiscoveredPattern, PatternExamples, ValidatedPattern
...
class TestValidateCommand:
    def test_validate_command_promotes_raw_to_validated(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr("lookout.discovery.storage.PatternStorage", mock_storage_cls)

        runner = CliRunner()
        with runner.isolated_filesystem():
            Path(".lookout-patterns").mkdir()
            result = runner.invoke(
                cli,
                ["validate", "ARCH-DI-001", "--patterns-dir", ".lookout-patterns"],
            )
    ...
    def test_validate_command_pattern_not_found(self, monkeypatch) -> None:
        ...
        with runner.isolated_filesystem():
            ...
            result = runner.invoke(
                cli,
                ["validate", "ARCH-DI-999", "--patterns-dir", ".lookout-patterns"],
            )
