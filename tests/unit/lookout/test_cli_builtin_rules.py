...
class TestCheckWithBuiltInRules:
    """Tests for 'lookout check' command with built-in rules."""
    ...
    def test_check_with_only_builtin_rules(self, tmp_path, monkeypatch):
        ...
        # Run lookout check with --format json for easier assertion
        runner = CliRunner()
    ...
    def test_check_gracefully_handles_missing_rules_dir(self, tmp_path, monkeypatch):
        ...
        # Create a test project directory without .lookout-rules
        project_dir = tmp_path / "test-project"
        project_dir.mkdir()
...
class TestCheckWithMixedRules:
    """Tests for 'lookout check' with both built-in and user rules."""
...
"""
        ...
        # Create lookout.toml config
        (project_dir / "lookout.toml").write_text('rules_dir = ".lookout-rules"\ntarget = ["."]\n')

        # Change to project directory
        monkeypatch.chdir(project_dir)

        # Run lookout check with JSON output
        runner = CliRunner()
        result = runner.invoke(cli, ["check", "--format", "json"])
...
class TestDiscoverWithBuiltInRules:
    """Tests for 'lookout discover' command with built-in rules."""
