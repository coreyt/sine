...
from lookout.semgrep import compile_semgrep_config, parse_semgrep_output
...
def test_parse_semgrep_output_maps_multi_segment_pattern_ids() -> None:
    spec = RuleSpecFile(
        schema_version=1,
        rule=RuleSpec(
            id="PY-SEC-001",
    ...
    assert len(findings) == 1
    assert findings[0].pattern_id == "PY-SEC-001"
