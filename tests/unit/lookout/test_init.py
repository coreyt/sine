"""Tests for lookout init command."""
...
class TestGenerateConfigContent:
    ...
    def test_generate_config_for_pyproject_toml(self):
        """Should generate [tool.lookout] section for pyproject.toml."""
        config = {
        ...
        content = generate_config_content(config, Path(".lookout-rules"))

        assert "[tool.lookout]" in content
        assert 'rules_dir = ".lookout-rules"' in content
        assert 'target = ["src", "tests"]' in content
        assert 'format = "text"' in content

    def test_generate_config_for_lookout_toml(self):
        """Should generate root-level config for lookout.toml."""
        config = {
            "config_file": "lookout.toml",
            "target": ["."],
            "format": "json",
        }
        ...
        assert "[tool.lookout]" not in content
        assert 'rules_dir = ".lookout-rules"' in content
...
class TestRunInit:
    ...
    def test_run_init_non_interactive_creates_config(self, tmp_path, monkeypatch):
        """Should create lookout.toml in non-interactive mode."""
        ...
        run_init(
            rules_dir=Path(".lookout-rules"),
            copy_built_in_rules=False,
            interactive=False,
        )

        # Should create lookout.toml
        config_file = project_dir / "lookout.toml"
    ...
    def test_run_init_creates_rules_directory(self, tmp_path, monkeypatch):
        """Should create .lookout-rules directory."""
        project_dir = tmp_path / "test-project"
        project_dir.mkdir()
        ...
        assert (project_dir / ".lookout-rules").exists()
