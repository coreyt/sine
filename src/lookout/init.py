"""Interactive initialization for Lookout configuration.
...
def prompt_user_config(project_info: dict[str, bool]) -> dict[str, Any]:
    ...
    Returns:
        ...
        - config_file: "pyproject.toml" or "lookout.toml"
    ...
    if project_info["python"]:
        ...
        config["config_file"] = "pyproject.toml" if use_pyproject else "lookout.toml"
    else:
        config["config_file"] = "lookout.toml"

    # Ask about target directories
    default_target = "src" if (Path.cwd() / "src").exists() else "."
    target = click.prompt(
        "Which directories should Lookout analyze?",
...
def run_init(
    ...
    """
    click.echo("Initializing Lookout for your project...\n")
    ...
    if interactive:
        ...
        config = {
            "config_file": "pyproject.toml" if project_info["python"] else "lookout.toml",
            "target": ["."],
            "format": "text",
        }
    ...
    if config["config_file"] == "lookout.toml" and config_file.exists():
        if interactive:
    ...
    if config["config_file"] == "pyproject.toml" and config_file.exists():
        ...
        with config_file.open("a") as f:
            f.write("\n" + config_content)
        click.echo(f"✓ Added [tool.lookout] section to {config['config_file']}")
    else:
        # Create new file
    ...
    # Final instructions
    click.echo("\n" + "=" * 50)
    click.echo("Lookout initialization complete!")
    click.echo("=" * 50)
    click.echo("\nNext steps:")
    ...
    click.echo("  3. Run 'lookout check' to start enforcing patterns")
    click.echo("\nFor more info: https://github.com/coreyt/lookout")
