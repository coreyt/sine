...
class WebSearchClient:
    ...
    def __init__(
        ...
        Args:
            ...
            search_provider: Pluggable search backend (required for actual searches)
        """
    ...
    async def __aexit__(
        ...
        """Async context manager exit — closes the search provider if present."""
    ...
    async def _execute_search(self, query: SearchQuery) -> list[dict[str, str]]:
        ...
        return await self.search_provider.execute_search(
            query.query,
            max_results=query.max_results,
            allowed_domains=query.allowed_domains if query.allowed_domains else None,
        )
