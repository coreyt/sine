...
def get_built_in_rules_path() -> Path:
    ...
    return Path(str(files(lookout) / "built_in_rules"))


def load_built_in_rules() -> list[RuleSpecFile]:
    """Load all built-in rule specifications.

    Built-in rules are shipped with the Lookout package in the
    src/lookout/built_in_rules/ directory and are always available.
...
def load_all_rules(user_rules_dir: Path | None) -> list[RuleSpecFile]:
    ...
    Example:
        ...
        # Load built-in + user rules from .lookout-rules/
        rules = load_all_rules(user_rules_dir=Path(".lookout-rules"))
    """
    # Load built-in rules
