...
from sine.discovery.extractors.llm import (
    API_ENDPOINTS,
    DEFAULT_MODELS,
    LLMProvider,
)
...
logger = logging.getLogger("sine.rule_generator")
...
class RuleGenerator:
    ...
    async def generate_check(self, pattern: ValidatedPattern) -> RuleCheck | None:
        ...
        Returns:
            A RuleCheck instance, or None on any failure
        """
        if not self._client:
...
1. Analyze the bad examples to determine what code pattern should be detected
2. Choose the single best check type for this pattern
3. Use Semgrep pattern syntax ($VAR for metavariables, ... for wildcards)
4. Return ONLY a single valid JSON object (no markdown wrapping, no explanation)

Return the JSON object now:"""

    def _parse_check(self, text: str) -> RuleCheck | None:
        """Parse LLM response text into a RuleCheck.

        Handles markdown wrapping and validates against Pydantic models.

        Returns:
            A RuleCheck instance, or None on parse/validation failure
        """
        text = text.strip()
        if not text:
            return None
